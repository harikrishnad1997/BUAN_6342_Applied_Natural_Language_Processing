...
class ManualFeatures(TransformerMixin, BaseEstimator):
    ...
    def spell_check_text(self, text):
        if self.spell_check:
            return [self.spell_checker.correction(word) for word in text]
        else:
            return text
    ...
    def transform(self, X, y=None):
        ...
        try:
            ...
            if self.spell_check:
                spelling_error_features = self.get_spelling_error_features(X)
                feature_names.append('spelling_errors')
